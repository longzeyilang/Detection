...
void LocalClubDet::resize_uniform(cv::Mat &src, cv::Mat &dst, cv::Size dst_size,
                              object_rect &effect_area) {}

void LocalClubDet::preprocess(cv::Mat &img, cv::Mat &roiImage,
  ...
  cv::Rect rec =
      cv::Rect(cv::Point(effect_area.x, effect_area.y), cv::Point(xmax, ymax));
  // cv::rectangle(img,rec,cv::Scalar(255,0,0));
  // static int a=1;
  // std::string fileName=std::to_string(a++)+".png";
...
void LocalClubDet::postprocess(const cv::Mat &bgr,
